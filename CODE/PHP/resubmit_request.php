...
if ($requestId <= 0 || $remarks === '') {
    $_SESSION['flash'] = ['type' => 'notice-error', 'message' => 'Request ID and correction instructions are required.'];
    header("Location: pending_requests.php");
    exit();
}
...
if ($stmt->execute()) {
    ...
    $_SESSION['flash'] = ['type' => 'notice-error', 'message' => 'Failed to update the request.'];
}
